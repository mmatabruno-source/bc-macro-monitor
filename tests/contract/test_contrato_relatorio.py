...
Payload real: tests/fixtures/relatorios_quantidade_5.json
...
FIXTURE = Path(__file__).resolve().parent.parent / "fixtures" / "relatorios_quantidade_5.json"
...
def test_parse_extrai_relatorio_mais_recente_primeiro():
    ...
    assert relatorios[0].identificador == "202412"
    assert relatorios[0].data_publicacao == "2024-12-19"
    assert relatorios[0].link_pdf.endswith("ri202412p.pdf")
    assert relatorios[0].link_pagina == "https://www.bcb.gov.br/publicacoes/ri/202412"
...
def test_parse_ordem_mais_recente_para_mais_antigo():
    ...
    assert identificadores == ["202412", "202409", "202406", "202403", "202312"]
